...
const corsMiddleware = (req, res, next) => {
    ...
    const isAllowedOrigin = (() => {
        ...
        if (isDevelopment) {
            ...
            return isLocal || ENV_CONFIG.development.allowedOrigins.some(pattern => new RegExp(`^${pattern.replace(/\*/g, '.*')}$`).test(origin));
        }
        // In testing, only allow test domains and ports
        if (isTesting) {
            return ENV_CONFIG.test.allowedOrigins.some(pattern => new RegExp(`^${pattern.replace(/\*/g, '.*')}$`).test(origin));
        }
