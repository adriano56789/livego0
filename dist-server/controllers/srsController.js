...
export const srsController = {
    ...
    deleteConnectionById: async (req, res, next) => {
        try {
            const { id } = req.params; // ID do cliente SRS
            const userId = req.userId;
            if (!userId) {
                return sendError(res, "Não autenticado.", 401);
            }
    ...
    getVersions: async (req, res) => sendSuccess(res, { version: "SRS/5.0 via LiveGo Service" }),
    getFeatures: async (req, res) => sendSuccess(res, { features: "rtc, rtmp, hls" }),
    ...
    getStreamById: (req, res) => sendError(res, "Not implemented via Service layer yet", 501),
    deleteStreamById: (req, res) => sendError(res, "Use deleteConnectionById to stop streams", 501),
    getConnections: (req, res) => sendError(res, "Use getClients instead", 501),
    getConnectionById: (req, res) => sendError(res, "Not implemented via Service layer yet", 501),
    getConfigs: (req, res) => sendError(res, "Protected", 403),
    updateConfigs: (req, res) => sendError(res, "Protected", 403),
    getVhosts: (req, res) => sendError(res, "Protected", 403),
    getVhostById: (req, res) => sendError(res, "Protected", 403),
    getRequests: (req, res) => sendError(res, "Protected", 403),
    getSessions: (req, res) => sendError(res, "Protected", 403),
    getMetrics: (req, res) => sendError(res, "Protected", 403),
};
