...
export const getApiDefinitions = (): Array<{ group: string, endpoints: EndpointDefinition[] }> => [
    ...
    },
    {
        ...
        endpoints: [
            ...
            { method: 'POST', path: '/users/me', func: api.users.update, params: [{ name: 'id', default: 'me' }, { name: 'data', default: '{"country":"en"}' }], buildPayload: (p: any) => JSON.parse(p.data), collections: ['users'] },
    ...
    },
    {
        ...
        endpoints: [
            ...
            // @FIX: Corrected the path for the RTC publish endpoint.
            { method: 'POST', path: '/rtc/v1/publish', func: api.srs.rtcPublish, params: [{ name: 'sdp', default: 'v=0...' }, { name: 'streamUrl', default: 'webrtc://test/live/test' }], buildPayload: (p) => ({ sdp: p.sdp, streamUrl: p.streamUrl }), collections: [] },
            { method: 'POST', path: '/rtc/v1/trickle/:sessionId', func: api.srs.trickleIce, params: [{ name: 'sessionId', default: 'rtc-session-123' }, { name: 'candidate', default: '{"candidate":"..."}' }], buildPayload: (p) => JSON.parse(p.candidate), collections: [] }
        ]
    },
];
