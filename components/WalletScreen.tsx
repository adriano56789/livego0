...
import { ChevronLeftIcon, MoreIcon, SolidDiamondIcon, CheckIcon, PixIcon, MercadoPagoIcon, ChevronRightIcon, YellowDiamondIcon } from './icons';
...
import CreditCardPaymentScreen from './card/CreditCardPaymentScreen';
...
// Definição dos tipos de visualização possíveis
type WalletView = 'main' | 'history' | 'withdraw_method' | 'purchase_method' | 'purchase_pix' | 'purchase_card';

const WalletScreen: React.FC<WalletScreenProps> = ({ onClose, initialTab = 'Diamante', currentUser, updateUser, addToast }) => {
    // Estado unificado de visualização para evitar conflitos de navegação
    ...
    // 1. Seleciona o pacote e vai para a tela de escolha de método
    const handlePackageSelect = (pkg: { diamonds: number; price: number }) => {
        setSelectedPackage(pkg);
        setCurrentView('purchase_method');
    };

    // 2. Vai para a tela de Pix
    const handleSelectPix = () => {
        setCurrentView('purchase_pix');
    };

    // 2. Vai para a tela de Cartão
    const handleSelectCreditCard = () => {
        setCurrentView('purchase_card');
    };

    // 3. Volta da tela de pagamento para a escolha de método
    const handleBackToMethodSelection = () => {
        setCurrentView('purchase_method');
    };

    // 4. Volta da escolha de método para a carteira principal (e limpa seleção)
    ...
    // Tela: Escolha do Método de Pagamento
    if (currentView === 'purchase_method' && selectedPackage) {
        return (
            <ConfirmPurchaseScreen 
                packageDetails={selectedPackage}
                onClose={handleBackToWallet}
                addToast={addToast}
    ...
    if (currentView === 'purchase_pix' && selectedPackage) {
        return (
            <PixPaymentScreen
                packageDetails={selectedPackage}
    ...
    // Tela: Pagamento via Cartão
    if (currentView === 'purchase_card' && selectedPackage) {
        return (
        ...
        return (
            <div className="fixed inset-0 bg-[#0A0A0A] z-[120] flex flex-col font-sans">
                <div className="flex items-center justify-between p-4 bg-[#0A0A0A]">
                    <button onClick={() => setCurrentView('main')} className="p-1">
                        <ChevronLeftIcon className="text-white w-6 h-6" />
                    </button>
