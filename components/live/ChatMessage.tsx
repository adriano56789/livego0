...
// A new, visually rich level badge component to replace the old one.
const LevelBadge: React.FC<{ level: number; className?: string }> = ({ level, className }) => {
    const getGradientColors = () => {
        ...
        if (level < 40) return ['#4ade80', '#16a34a']; // Green to Darker Green
        if (level < 60) return ['#f87171', '#dc2626']; // Red to Darker Red
        return ['#f5f5f5', '#9ca3af']; // White to Gray for high levels
    };
    ...
    // Adjust font size for 3-digit numbers
