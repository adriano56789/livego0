...
export const mercadoPagoController = {
    ...
    createCardPreference: async (req: AuthRequest, res: ExpressResponse) => {
        try {
            ...
            if (!mpResponse.ok) throw new Error(data.message || 'Falha ao criar preferência de pagamento.');
    ...
    createPixPayment: async (req: AuthRequest, res: ExpressResponse) => {
        try {
            const { details } = req.body;
            const { diamonds, price } = details;
            const userId = req.userId;
    ...
    webhook: async (req: ExpressRequest, res: ExpressResponse) => {
        const payment = (req as any).query;
        ...
        try {
            if (payment.type === 'payment' || payment.topic === 'payment') {
                ...
                if (!paymentId) return (res as any).status(200).send('OK');
                ...
                if (!paymentInfoResponse.ok) return (res as any).status(200).send('OK'); 

                const paymentInfo: any = await paymentInfoResponse.json();
                const internalTxId = paymentInfo.external_reference;
                const status = paymentInfo.status;

                if (!internalTxId) return (res as any).status(200).send('OK');

                const transaction = await TransactionModel.findOne({ id: internalTxId });
                if (!transaction) return (res as any).status(200).send('OK');
                ...
                if (transaction.status !== 'completed' && status === 'approved') {
                    ...
                    await UserModel.findOneAndUpdate(
                        ...
                        { $inc: { diamonds: transaction.amountDiamonds } }
                    );
            ...
            (res as any).status(500).send('Erro');
        }
    }
};